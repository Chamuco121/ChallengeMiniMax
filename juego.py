...
TABLERO_TAMAŃO = 6
TAMANIO_CELDA = 60
ANCHO_VENTANA = TABLERO_TAMAŃO * TAMANIO_CELDA
ALTO_VENTANA = TABLERO_TAMAŃO * TAMANIO_CELDA
...
tablero = np.zeros((TABLERO_TAMAŃO, TABLERO_TAMAŃO))
...
def generar_destino(raton_pos, min_distancia):
    while True:
        #Genera una posición aleatoria para el destino del ratón que esté a una distancia mínima de la posición actual del ratón.
        destino = (random.randint(0, TABLERO_TAMAŃO - 1), random.randint(0, TABLERO_TAMAŃO - 1))
...
def mover_jugador(tablero, posicion_actual, nueva_posicion):
    if (0 <= nueva_posicion[0] < TABLERO_TAMAŃO) and (0 <= nueva_posicion[1] < TABLERO_TAMAŃO):
...
# Cargar imágenes GIF y redimensionarlas
    ...
    while corriendo and not juego_terminado(tablero):
        ...
        # Dibujar el tablero
        for x in range(TABLERO_TAMAŃO):
            for y in range(TABLERO_TAMAŃO):
